...
public class GameManager : MonoBehaviour
{
    ...
    {
        if (turnoActual == Turno.Player1) return player1;
        if (turnoActual == Turno.Player2) return player2;
        return null;
    }
    ...
    void TurnoEnemy()
    {
        ...
        Vector2Int start = GraphCreator.Instance.WorldToGrid(enemy.transform.position);
        Vector2Int goal = GraphCreator.Instance.WorldToGrid(objetivo.transform.position);
    ...
    {
        float d1 = Vector3.Distance(enemy.transform.position, player1.transform.position);
        float d2 = Vector3.Distance(enemy.transform.position, player2.transform.position);
